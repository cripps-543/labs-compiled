...
if __name__ == '__main__':
    ...
    for epoch in range(1, 100):
        ...
        for i, (images, labels) in tqdm(enumerate(trainloader), total=len(trainloader), leave=False):
            ...
            pred_labels_ce.extend(predicted_ce.cpu().numpy())
            pred_labels_nll.extend(predicted_nll.cpu().numpy())
        ...
        plt.ylabel('Loss')
        plt.legend()
        plt.savefig('task7_loss_plot.png')
        
        # Create and save the confusion matrix
        cm_ce = confusion_matrix(true_labels, pred_labels_ce)
        cm_nll = confusion_matrix(true_labels, pred_labels_nll)
        
        plt.figure(figsize=(8, 6))
        plt.subplot(1, 2, 1)
        plt.title('CrossEntropyLoss Confusion Matrix')
        plt.imshow(cm_ce, cmap='Blues')
        plt.colorbar()
        plt.xlabel('Predicted')
        plt.ylabel('True')
        
        plt.subplot(1, 2, 2)
        plt.title('NLLLoss Confusion Matrix')
        plt.imshow(cm_nll, cmap='Blues')
        plt.colorbar()
        plt.xlabel('Predicted')
        plt.ylabel('True')
